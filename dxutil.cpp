...
//-----------------------------------------------------------------------------
// Name: SlotMachineUtil_GetMediaPath()
// Desc: Returns the Drive Letter for all media paths
//-----------------------------------------------------------------------------
const TCHAR* SlotMachineUtil_GetMediaPath()
{
    ...
    LONG lResult = RegOpenKeyEx( HKEY_LOCAL_MACHINE,
                                _T("Software\\SlotMachine\\EliteKeno"),
                                0, KEY_READ, &hKey );
    if( ERROR_SUCCESS != lResult )
        return strNull;

    lResult = RegQueryValueEx( hKey, _T("EliteKeno Media Drive"), NULL,
...
//-----------------------------------------------------------------------------
// Name: SlotMachineUtil_FindMediaFile()
// Desc: Returns a valid path to a Slot Machine media files
//-----------------------------------------------------------------------------
HRESULT SlotMachineUtil_FindMediaFile( TCHAR* strPath, TCHAR* strFilename )
{
    HANDLE file;
    ...
    // Check if the file exists in the current directory
    _stprintf( strPath, _T("%s%s"), SlotMachineUtil_GetMediaPath(), strFilename );
