//----------------------------------------------------------------------------------------------
//
// ddutil.cpp: Defines the application's DirectDraw framework classes use to construct
//			   all of the various graphical DirectX drawing surfaces and Device Contexts.
//
...
HRESULT CSurface::Clear(DWORD dwFillColor)
{
    // Erase the background
    DDBLTFX ddbltfx;
    ZeroMemory( &ddbltfx, sizeof(ddbltfx) );
...
//-----------------------------------------------------------------------------
// Name: CSurface::DrawBitmapPart()
// Desc: Draws a bitmap over a part of the DirectDrawSurface, blitting the 
//       bitmap without stretching it.
//-----------------------------------------------------------------------------
HRESULT CSurface::DrawBitmapPart( HBITMAP hBMP, DWORD dwDestX, DWORD dwDestY,
								  DWORD dwBMPOriginX, DWORD dwBMPOriginY, 
								  DWORD dwBMPWidth, DWORD dwBMPHeight )
{
    HDC            hDCImage;
    ...
    // Draw the bitmap in the RECT for this surface
    if( FAILED( hr = m_pdds->GetDC( &hDC ) ) )
        return hr;

    if( FAILED( hr = BitBlt( hDC, dwDestX, dwDestY, dwBMPWidth, dwBMPHeight, hDCImage, dwBMPOriginX, dwBMPOriginY, SRCCOPY )))
		return hr;
